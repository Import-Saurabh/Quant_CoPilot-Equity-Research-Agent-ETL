...
# ==========================================
# 1. BALANCE SHEET SCRAPER
# ==========================================

def scrape_balance_sheet(ticker):
    print(f"\n=== Starting Balance Sheet Extraction for Ticker: {ticker} ===")
    ...
    print("\n--- Balance Sheet Child Schedules ---")
    
    for parent in schedule_parents:
        print(f"\nQuerying: [{parent}]...")
        child_items = fetch_schedule_item(screener_id, parent, section="balance-sheet")
...
# ==========================================
# 2. PROFIT & LOSS SCRAPER
# ==========================================

def scrape_profit_loss(ticker):
    print(f"\n=== Starting Profit & Loss Extraction for Ticker: {ticker} ===")
    ...
    if not pl_section:
        print("[ERROR] Could not find Profit & Loss section.")
        return
        
    table = pl_section.find("table", class_="data-table")
    dates, main_rows = parse_html_table(table)
    
    print("\n--- Profit & Loss Parent Rows ---")
    print(f"Timeline Columns: {dates}")
    for label, values in main_rows.items():
        print(f"  {label.ljust(35)}: {values}")
        
    schedule_parents = ["Expenses", "Other Income", "Net Profit"]
    print("\n--- Profit & Loss Child Schedules ---")
...
def scrape_cash_flow(ticker):
    print(f"\n=== Starting Cash Flow Extraction for Ticker: {ticker} ===")
    ...
    if not cf_section:
        print("[ERROR] Could not find Cash Flow section.")
        return
        
    table = cf_section.find("table", class_="data-table")
    dates, main_rows = parse_html_table(table)
    
    print("\n--- Cash Flow Parent Rows ---")
    print(f"Timeline Columns: {dates}")
    for label, values in main_rows.items():
        print(f"  {label.ljust(35)}: {values}")
    ...
    print("\n--- Cash Flow Child Schedules ---")
    
    for parent in schedule_parents:
        print(f"\nQuerying: [{parent}]...")
        child_items = fetch_schedule_item(screener_id, parent, section="cash-flow")
...
if __name__ == "__main__":
    ...
    # Run the discrete functions
    # scrape_balance_sheet(ticker_to_test)
    # scrape_profit_loss(ticker_to_test)
    scrape_cash_flow(ticker_to_test)